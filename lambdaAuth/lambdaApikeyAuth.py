...
#The sole purpose of this function is to extract & return the apiKey param. '^[/._@a-zA-Z0-9-\*]+$'
def lambda_handler(event, context):
    print(event)

    # The following code tries to retrieve the apiKey value from headers or request parameters
    try:
        apiKey = event['headers']['apiKey']
    except Exception:
        print("apiKey header does not exist in the request")
        try:
            apiKey = event['headers']['apikey']   #get the value using apikey lower cased
        except Exception:
            print("apikey (lower cased) header does not exist in the request")
            try:
                apiKey = event['headers']['Apikey']  #get the value using apikey lower cased
            except Exception:
                print("apiKey header does not exist in the request")
